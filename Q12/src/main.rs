...
const N: u32 = 140; // size: 140X140

fn helper(
    x: i32,
    y: i32,
    to_visit: &HashSet<(i32, i32)>,
    matrix: &[[char; N as usize]; N as usize],
    ch: char,
) -> (bool, bool) {
...
fn explore(
    ...
    circum: &mut u32,
    area: &mut u32,
    to_visit: &mut HashSet<(i32, i32)>,
    matrix: &[[char; N as usize]; N as usize],
    barriers: &mut HashSet<(i32, i32, u32)>,
) {
    *area += 1;
    to_visit.remove(&(i, j));
    //print!("({i}, {j}) -> ");
    ...
    if up.0 {
        ...
        // Both left and right: meaning we got an extra barrier
        if barriers.contains(&(i, j - 1, 0)) && barriers.contains(&(i, j + 1, 0)) {
            *circum -= 1;
        }
    ...
    if up.1 {
        explore(i - 1, j, circum, area, to_visit, matrix, barriers);
    }
    ...
    if right.1 {
        explore(i, j + 1, circum, area, to_visit, matrix, barriers);
    }
